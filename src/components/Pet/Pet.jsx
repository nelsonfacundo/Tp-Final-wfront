...
const PetCard = ({ pet, showAdoptButton }) => {
  ...
  const adoptar = async () => {
    ...
    setIsAdopting(true); // Deshabilitar botones durante la solicitud de adopción
    try {
      ...
      if (response.ok) {
        setMessage({ text: `Solicitud de adopción enviada`, type: "success" });
        setTimeout(() => {
          // Refrescar la página después de 2 segundos
          window.location.reload();
        }, 2000);
      } else {
        setMessage({ text: "Solicitud de adopción fallida", type: "error" });
      }
    } catch (error) {
      console.error("Error: ", error);
    } finally {
      setIsAdopting(false); // Habilitar botones después de la solicitud de adopción
    }
  };
  ...
  return (
    <div className={`petCard ${isDeleted ? "petCardDeleted" : ""}`}>
      ...
      {showAdoptButton && isAuthenticated() && !isAdmin() && (
        <button onClick={() => adoptar(_id)} disabled={isAdopting}>
          Adoptar
        </button>
      )}
      {!showAdoptButton && isAuthenticated() && isAdmin() && !isDeleted && (
